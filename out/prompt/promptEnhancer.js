...
class PromptEnhancer {
     ...
     */
    async enhancePrompt(userPrompt, role, modelId) {
     ...
     */
    addContextEnhancement(prompt, context) {
        const enhancements = [];
        let contextInfo = '';
        // Add file context
     ...
     */
    addRoleEnhancement(prompt, role) {
        const roleMetadata = defaults_1.DEFAULT_ROLES.find(r => r.name === role);
        if (!roleMetadata) {
     ...
     */
    addCodePatternSuggestions(prompt, role, context) {
        const patterns = {
            'Laravel Developer': [
